...
class Game(object):
    ...
    def get_forest(self):
        return [forest for forest in self.forests]

    def get_mines(self):
        return [mine for mine in self.gold_mines]
    ...
    def move(self, direction):
        ...
        if row >= 0 and row < self.n and col >= 0 and col < self.n:
            ...
            for k in range(len(self.gold_mines)):
                ...
                self.gold_mines_next[k] = mine.is_reached(destination)
            for k in range(len(self.forests)):
                forest = self.forests[k]
                self.forests_next[k] = forest.is_reached(destination)
            self.chest_next = self.chest.is_reached(destination)
